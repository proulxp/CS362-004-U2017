 ...
 ** Description: An oracle function called checkVillageCard() this function
 checks the execution of the village card using a set of random inputs
 in the passed game state.
 
 Referenced: checkDrawCard() in testDrawCard.c
 
 *********************************************************************/
int checkVillageCard(int playerIndex, int testHandPos, struct gameState *afterState) {
    ...
    //Call cardEffect on village card
    cardEffect(village, 0, 0, 0, afterState, testHandPos, 0);
    ...
    //Simulate Action gain mechanic
    
    beforeState.numActions = beforeState.numActions + 2;
...
int main () {
    ...
    int playerIndex;    //Random player index to test checkVillageCard()
    ...
    for (numTests = 0; numTests < 2000; numTests++) {
        ...
        checkVillageCard(playerIndex, testHandPos, &GS);
    }
